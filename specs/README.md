...
- **Post-approval:** settle it. Research it first; if research produces a clear winner, record `DECIDED (research, <date>)` with sources. If it does not, walk the **post-approval tie-breaker ladder** (approved feature spec, then upstream standards and dependency docs, then the existing codebase pattern, then reversibility, then smallest scope) and take the first rung that discriminates. A tie broken on the last two rungs is recorded honestly as `ASSUMED (story prep, <date>)`, naming the alternative and what reversing it would cost.
