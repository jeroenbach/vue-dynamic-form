...
>
> CORRECTED (Jeroen, `/spec:discuss ST-01`): the premise above is wrong and the decision is superseded. `defineMetadata.ts` only *auto-injects* `default` (`ValueTypeMap = FieldValueTypes & { default: string }`); it never excludes a consumer's own `default` key, so there was no existing "already excluded" mechanism for `wizard` to join. No type-level exclusion is added for `wizard` either: it gets exactly the treatment `default` already gets, no `@ts-expect-error`, no change to `defineMetadata.ts`. The `wizard-wizard` slot-name collision is avoided at the docs-authoring level (the rebuild simply doesn't use `type: 'wizard'`), not by a type-level guard. See ST-01's AC16 for the corresponding story-level correction.
...
- `AdvancedFormTemplate.vue`: replace the `#wizard` slot with a `#default-wizard` slot bound to the new `WizardAttributes` (the node drops its `type: 'wizard'` marker as a docs-authoring choice per ADR 5's CORRECTED note, not because the name is type-level reserved); replace `#wizardPage`, `#wizardPage-array`, `#wizardPage-array-item`, `#wizardPage-choice`, `#wizardPage-choice-array` with a single `#default-wizard-page` (or `<type>-wizard-page`) visibility wrapper that `v-show`s on `isCurrent`; drop the `currentStepIndex`/`gotoStepIndex` entries from the `defineMetadata` fourth generic and the manual `v-show="... === index"` per-page gating.
...
4. **[nit] ADR 5's ladder safety analysis holds only under the camelCase-type convention and should say so.** A userland field type literally ending in `-wizard` or `-wizard-page` (kebab) would misroute to `default-wizard` / `default-wizard-page`, exactly as a type ending in `-array` misroutes to `default-array` today. This is safe in practice only because CLAUDE.md mandates camelCase field types (`myWizard`, `wizardPage` do not contain the `-wizard`/`-wizard-page` substring at a suffix boundary), so the trap is unreachable without violating the convention. ADR 5 should state this reliance explicitly rather than implying the suffixes are collision-proof in the absolute.
   ...
   >
   > CORRECTED (Jeroen, `/spec:discuss ST-01`): the "reserved at the type level" half is superseded. See ADR 5's CORRECTED note: no type-level exclusion is added for `wizard`, it typechecks exactly as `default` does today.
...
5. **[nit] A wizard node that keeps `type: 'wizard'` produces the container slot name `wizard-wizard`.** The current example marks the node with `type: 'wizard'` (`FormExampleClientOnboardingPlanner.vue:206`); once `wizard: true` is the marker, the rebuild should drop or repurpose that `type` so the container dispatches through `default-wizard` (or an intentional `<type>-wizard`) rather than the awkward `wizard-wizard`. Harmless, but worth calling out for the rebuild story.
   ...
   >
   > CORRECTED (Jeroen, `/spec:discuss ST-01`): the rebuild still drops `type: 'wizard'` from the example, but `wizard` does not become a reserved type name in `defineMetadata`; no type-level mechanism is added. See ADR 5's CORRECTED note.
...
Split into a single story, per Jeroen's stated preference (see Constraints & assumptions, "Single-story delivery"): the two natural seams recorded in Architecture's "Natural slicing seams" are not carved into separate stories, so the whole feature lands as one implementation and one verification pass.

| Story | Scope | Depends on |
| --- | --- | --- |
| [ST-01](stories/ST-01-wizard-metadata-shape/spec.md) | The whole feature: `FieldMetadata.wizard`, `isWizard` detection, `DynamicFormItemWizard`, the `-wizard`/`-wizard-page` slot families and their types, the docs onboarding example rebuild (`FormExampleClientOnboardingPlanner.vue`/`FormWizard.vue`/`AdvancedFormTemplate.vue`), retirement of `registerWizardPagePath`/`wizardPagePaths`/`validatePage`/`submitForm`/`loadingResolve.ts`, and the four doc-page updates. | none |

Implementation order: ST-01 only.
