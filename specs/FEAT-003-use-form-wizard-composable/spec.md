...
- AR-1 (blocker) → path derivation must replicate all three parts of `DynamicForm.vue:76`, `fieldMetadata.path ??` first (ADR-1 fixed; verified against the actual file)
- ADR-1 alternative → keep the two-file duplication over a shared internal helper, to stay clear of the Out-of-scope fence on `correctMetadataAndSetDefaults`
- AR-3 (should-fix) → the loadingResolve grep was incomplete; `docs/reference/use-validate-partial-form.md` is a fourth caller, folded into Slice B's already-planned update of that page
- AR-4 (should-fix) → `FormWizard.vue`'s `Stepper` reads `wizard.pages` instead of re-deriving from `fieldMetadata.children`, so there's one source of truth for step count
- AR-5 (should-fix) → document the one-wizard-per-provider-subtree constraint (Q5's doc + `specs/components.md`); no signature reservation needed, the options-bag argument already covers future additions
...
- Documentation updates: `docs/examples/advanced.md` (the onboarding wizard walkthrough), `docs/reference/use-validate-partial-form.md` (remove its `registerWizardPagePath`/`wizardPagePaths`/`LoadingResolve`/`validatePage` worked example, per Open question 5), and a new dedicated reference page `docs/reference/use-form-wizard.md` (alongside `dynamic-form.md`, `use-validate-partial-form.md`), per Open question 5's decision.
...
```ts
const { validateSection, handleSubmit } = useDynamicForm<Values>();
const wizard = useFormWizard(wizardNode.children, { validateSection, onSubmit: handleSubmit(onValid) });
// wizard.currentStepIndex, wizard.pages (label/description/path per page),
// wizard.next(), wizard.prev(), wizard.gotoStep(i), wizard.submit(),
// wizard.isFirst, wizard.isLast, wizard.isValidating
```

- `wizard.pages` is derived from the wizard node's `children` array the consumer passes in, giving each page's path without a `computedProps` side effect.
...
- `wizard.submit()` wraps the supplied `onSubmit` (typically the app's `handleSubmit(...)` result), so `FormWizard.vue` can call it directly instead of needing a submit transport of its own.
...
- Retire `docs/.vitepress/theme/utils/loadingResolve.ts`. Its callers are `AdvancedFormTemplate.vue`, `FormExampleClientOnboardingPlanner.vue`, `FormWizard.vue`, and (via the type import only) `docs/reference/use-validate-partial-form.md:68`'s worked wizard example. Deleting the util requires Slice B to also remove the `LoadingResolve`/`validatePage` snippet from that reference page, not only its `registerWizardPagePath`/`wizardPagePaths` pattern.
  > DECIDED (research): re-ran the grep across the whole repo (not just the three docs components); it confirms `docs/reference/use-validate-partial-form.md:68` is a fourth caller the original grep missed. Corrected above and folded into Slice B's scope, which already covered updating that file (Scope section, Q5) — this only adds one more thing for that pass to remove. No consumer-visible or public-API question here, so it does not need Jeroen's input.
  > Q3 (submit ownership) is now DECIDED as option (a), the composable owns `submit()` (see Open questions and the Q3 seam below), which is exactly what keeps this deletion unconditional: the composable's inject channel is the transport that replaces `loadingResolve`, so no caller of the old util survives the refactor.
...
- **Single source of truth for step count:** `FormWizard.vue` derives its `Stepper` steps from the injected `wizard.pages`, not by independently re-mapping `fieldMetadata.children` (as it does today at `AdvancedFormTemplate.vue:89`).
  > DECIDED (research): there are three derivations of "the steps" that must agree in count and order: the composable's `pages` (drives `isFirst`/`isLast`), `FormWizard.vue`'s `Stepper` steps, and the engine's render `index` (drives the `v-show` gating). They agree today only because all three happen to enumerate the same `children` array; if Q1's answer ever makes `pages` a filtered or explicit subset, a `Stepper` that still re-derives from `fieldMetadata.children` would desync from the composable's `isLast`, landing the submit button on the wrong page. Pinning `FormWizard.vue` to read `wizard.pages` removes a second derivation with no functional downside (the demo's stepper looks identical either way, since today's `children` map and the composable's `pages` cover the same set) and is invisible to a consumer of the library. Whatever Q1 decides, this makes it authoritative in one place instead of two.
...
- **`gotoStep(index)` policy (Q2 — DECIDED):** backward-only by default (`index <= currentStepIndex.value`), matching today's docs behaviour exactly; also still enforced independently by `Stepper.vue`'s `:disabled="i > currentStep"` as a second gate. An `allowForwardJump`/`validateOnJump` option lets a consumer opt into forward jumping; the demo does not set it, so its behaviour is unchanged by the refactor.
- **`submit()` ownership (Q3 — DECIDED, option (a)):** the composable owns it. `useFormWizard(pages, { validateSection, onSubmit })`, where `onSubmit` wraps the consumer's `handleSubmit(...)`; `FormWizard.vue` calls `await wizard.submit()`. This resolves the transport gap adversarial-review finding 2 raised under option (b): `FormWizard` is instantiated inside `AdvancedFormTemplate`'s `#wizard` slot, not by the app component, so a submit handler could not have reached it as a prop once `submitForm`-in-metadata and `loadingResolve` were both removed. Owning submit gives it a channel via the same inject object that already carries `next()`/`prev()`.
- **`isValidating` (Q4 — DECIDED):** included in the committed return type from the start. `next()` and `submit()` are the only places a validation/submit promise is in flight, so it is a small internal addition (set true before the `await`, false after) with no ripple elsewhere. This is the direct replacement for the capability `createLoadingResolve`'s `isRunningRef` used to provide.

### ADR notes

**ADR-1 — Replicate the path-join logic in the composable rather than reach the engine's corrected tree.**
Context: page paths must match what `correctMetadataAndSetDefaults` produces, but that corrected tree is internal to `DynamicForm` and never exposed. Decision: replicate the full three-part derivation exactly as `DynamicForm.vue:76` runs it, honouring the explicit-`path` precedence first: `fieldName = fieldMetadata.name ?? 'field-${index}'`, then `fieldMetadata.path ?? [parent?.path, fieldName].filter(Boolean).join('.')`. Covered by a unit test that pins the derivation, including (a) a node with an explicit `path` (the wizard root's `path: ''`) and (b) the resulting empty-root-prefix case.

> DECIDED (research): the derivation is three-part, not two, and the omitted part is load-bearing. Verified directly against `packages/core/src/components/DynamicForm.vue:76`: `const fieldPath = fieldMetadata.path ?? [parent?.path, fieldName].filter(Boolean).join('.')` — an explicit `path` on a node wins before the join. The demo's wizard root sets `path: ''` explicitly (confirmed in `FormExampleClientOnboardingPlanner.vue`), and the whole data-flow narrative depends on that `''` being the parent prefix. Replicating only the two-part join recomputes the root name as `'wizard'` and derives every page path as `wizard.<child>`, matching no vee-validate-registered path, so `validateSection` would return `valid: true` over zero fields and `next()` would advance without validating. This is a factual correction to the architecture text with one unambiguous right answer (match the engine exactly) and no consumer-visible or public-API impact, so it does not need Jeroen's input.
> On the alternative also raised (extracting a shared internal helper so `DynamicForm.vue` and the composable can never drift): DECIDED (research) to keep the two-file duplication as the shipped default, not the shared helper. Why: the shared helper touches `correctMetadataAndSetDefaults` internals, which this spec's Out-of-scope section explicitly fences ("Any change to how `DynamicForm` walks or corrects the metadata tree"); the duplicated three-part logic is trivial, has been stable for the engine's whole life, and is now pinned by the unit test above, so drift risk is low and the fence stays intact. Reversible later (extracting a helper afterward is a non-breaking internal refactor) at low cost, so this is not a one-way door.
...
Context: the consumer should not have to remember a second `provide()` step. Decision: `useFormWizard` calls `provide()` internally as part of setup. Alternative considered: return the object and require the consumer to `provide` it. Rejected: extra boilerplate and an easy-to-forget step; `provide` inside a composable run during `setup` is a supported Vue pattern (verified). Consequence: this assumes one wizard per provider subtree; nested/sibling wizards under one component would collide on the Symbol. Out of scope for the demo (single wizard), and documented as a known constraint (below) rather than solved now.
  > DECIDED (research): keep the single-Symbol default (unchanged), and document the one-wizard-per-provider-subtree constraint in the reference doc (whichever page Q5 lands it on) and in `specs/components.md`, since a second wizard or a nested `DynamicForm`-with-wizard would otherwise silently inject the wrong outer wizard with no error. On the reviewer's second suggestion (reserve a parameter position for a future optional injection key): not needed as a separate step. `useFormWizard(metadata, options)` already takes an options bag as its second argument, and a future `injectionKey` field added to that bag is additive by the same rule as every other option in this spec (new optional field, safe default) — there is no signature to reserve. This is an implementation/documentation detail with one clear right answer, invisible to how a consumer uses the composable today, so it does not need Jeroen's input. It does not affect the minor changeset bump either way.
...
- **Slice B — docs refactor + docs pages:** rebuild `FormExampleClientOnboardingPlanner.vue`, `FormWizard.vue`, `AdvancedFormTemplate.vue` on the composable, delete `loadingResolve.ts`, update `docs/examples/advanced.md` and `docs/reference/use-validate-partial-form.md` (remove the `registerWizardPagePath`/`wizardPagePaths`/`LoadingResolve`/`validatePage` teaching pattern), and add the new `docs/reference/use-form-wizard.md` page. Depends on Slice A. This is the "behaves identically to a reader" regression surface; the `launchApproach` choice page doubles as the choice-page regression check named in Constraints.
- Q1–Q5 are all now DECIDED (see Open questions): Q1 shapes Slice A's first argument (wizard node's `children`); Q2–Q4 shape Slice A's options/return (backward-only default, composable-owned `submit()`, committed `isValidating`); Q5 shapes Slice B's doc placement (new `use-form-wizard.md` page). None forces a third slice.
...
All resolved during `/spec:discuss`: the five Open questions are decided (see Open questions section), the ADR-1 fence tension is settled in favor of the two-file duplication (no amendment to Out-of-scope needed), and ADR-4's one-wizard-per-provider constraint is accepted as a documented limitation rather than solved in this feature.
...
- Page-path derivation must stay behaviorally consistent with `correctMetadataAndSetDefaults`'s own name-defaulting and path-joining (`packages/core/src/components/DynamicForm.vue:75-76`: `fieldMetadata.name ?? 'field-${index}'`, `fieldMetadata.path ?? [parent?.path, fieldName].filter(Boolean).join('.')`), since the whole point is that a page's derived path matches the path `DynamicFormItem` actually registers with vee-validate.
  > DECIDED (research): this bullet already stated the correct three-part derivation (including `fieldMetadata.path ??`); the drift was in ADR-1's paraphrase, now fixed there. `useFormWizard` reimplements this same joining logic against the raw metadata the consumer passes it (see ADR-1); it does not reach `DynamicForm`'s internal corrected tree, consistent with the Out-of-scope fence.
...
1. **What counts as "a page"?** Deriving pages from the metadata root's direct children fits the onboarding example (the root `wizard` node has path `''`, and its children are the pages), but a wizard whose relevant node is not the tree root, or whose pages sit at a deeper level, needs an escape hatch. Candidate shapes for the composable's first argument: (a) the full metadata array/tree plus an implicit "root's children are pages" rule, (b) the wizard node's `children` array directly, (c) an explicit array of page paths/labels the consumer supplies. Jeroen decides the API shape; this also determines how much of `correctMetadataAndSetDefaults`'s path-joining logic the composable needs to duplicate (see the Constraints note above).
   > DECIDED (Jeroen): option (b), the wizard node's `children` array directly: `useFormWizard(wizardNode.children, opts)`. Generalizes to a wizard at any depth with no implicit convention. See Functional overview and Architecture for the updated call shape.
2. **Navigation policy.** The docs example's `gotoStep` is backwards-only (`_index <= currentStepIndex.value`); that is the demo's choice, not a library rule. Should `useFormWizard` take options such as `allowForwardJump` or `validateOnJump`, and what should the default be (backwards-only, matching today's docs behavior, or something else)?
   > DECIDED (Jeroen): backward-only by default, matching today's docs behavior. A consumer who wants forward-jumping opts in via an `allowForwardJump`/`validateOnJump` option.
3. **Submit integration.** Should `useFormWizard` also own a `submit()` step that wires `handleSubmit` (mirroring how it wires `validateSection` for `next()`), or should it stop at navigation and leave `handleSubmit` entirely to the app component, as the proposed shape above assumes?
   > DECIDED (Jeroen): option (a), the composable owns `submit()`: `useFormWizard(pages, { validateSection, onSubmit })` wraps the consumer's `handleSubmit`; `FormWizard.vue` calls `wizard.submit()`. This also resolves finding 2 below: the deletion of `loadingResolve.ts` is unconditional now, since option (a) is exactly what makes it so.
4. **Loading/validating state.** Should the composable expose an `isValidating` (or similarly named) ref so a template can show a spinner or disable the Next button while `validateSection` is in flight? This is the direct replacement for what `createLoadingResolve`'s `isRunningRef` gave the old plumbing; without it, the retiring of `loadingResolve.ts` loses a capability the docs example currently has no visible use for today but could plausibly want.
   > DECIDED (Jeroen): yes, include `isValidating` in the committed return type from the start (not a later addition).
5. **Reference documentation placement.** Does `useFormWizard` get its own new page under `docs/reference/` (matching the existing one-composable-per-page pattern of `use-validate-partial-form.md`), or is it documented as a section within an existing reference page? Either way, `docs/reference/use-validate-partial-form.md`'s current worked wizard example (which demonstrates exactly the `registerWizardPagePath`/`wizardPagePaths` pattern this feature retires) needs to change so it does not keep teaching the pattern this feature replaces.
   > DECIDED (Jeroen): a new dedicated page, `docs/reference/use-form-wizard.md`, matching the existing one-composable-per-page pattern.

## Stories

Split by scrum-master along the two slicing seams named above. Implementation order matches dependency order; none can be reordered.

| Story | Rationale | Depends on |
| --- | --- | --- |
| [ST-01: Core `useFormWizard` composable](stories/ST-01-core-composable/spec.md) | Slice A in full: the published, independently testable engine surface (`useFormWizard`, `useFormWizardContext`, types, unit tests, changeset, `specs/components.md`). Ships value to any library consumer with no docs dependency. | None |
| [ST-02: Rebuild the onboarding wizard components on `useFormWizard`](stories/ST-02-docs-wizard-refactor/spec.md) | First half of Slice B: rewires `FormExampleClientOnboardingPlanner.vue`, `FormWizard.vue`, `AdvancedFormTemplate.vue` onto the composable and deletes `loadingResolve.ts`. Split out from the docs-pages work so the code refactor (with its own regression bar: the live example must behave identically) lands and is verified before the pages that describe it are written. | ST-01 |
| [ST-03: `useFormWizard` reference page and doc updates](stories/ST-03-docs-reference-pages/spec.md) | Second half of Slice B: the new `docs/reference/use-form-wizard.md` page plus updates to `docs/examples/advanced.md` and `docs/reference/use-validate-partial-form.md`. Kept separate from ST-02 because it is pure prose/reference work with a different regression surface (dead links, stale teaching content) and depends on ST-02's shipped result to describe accurately. | ST-01, ST-02 |

All five Open questions (Q1-Q5) are resolved in the DECIDED entries above and are already reflected in each story's acceptance criteria; no story reopens them.
