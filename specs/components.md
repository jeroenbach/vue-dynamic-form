...
`FieldMetadata` also gained `preserveOrder?: boolean` (additive, default off), meaningful on a `maxOccurs > 1` explicit choice branch whose occurrences are objects (declare `children`): opts into writing a persisted `order` field into each occurrence's own values, surviving reload and loaded saved data, at the price of `order` becoming part of the submitted shape (see `DynamicFormItemChoice` above). Static metadata only, excluded from `ComputedPropsFieldType` alongside `displayOrder`/`explicitChoiceSelection`/`preserveOnSwitch`, for the same reason: flipping it mid-form would leave existing occurrences with a stale or missing `order`.
