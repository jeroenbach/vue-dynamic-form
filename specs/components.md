...
`DynamicFormItemProps` also gained `globalIndex?: number` (additive, engine-internal wiring, mirroring `branchKey`): set alongside `branchKey` on a repeatable explicit choice occurrence's `DynamicFormItem`, it is the render-loop index over `activeChoiceOccurrences` and is forwarded to the slot as `globalIndex`. `undefined` everywhere else (plain array items, non-explicit choices, `maxOccurs: 1` explicit choices, and any other field).
