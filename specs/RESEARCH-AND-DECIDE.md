...
Research the point first, exactly as in step 1. If research produces a clear winner, record it as `DECIDED (research)` and move on. If it does not, **do not stop and do not guess**. Walk this ladder in order and take the first rung that discriminates between the options:
...
Rungs 1 to 3 produce evidence, so they record as `DECIDED (research)` with the source. Rungs 4 and 5 do not: they break a genuine tie by policy, and they record honestly as `ASSUMED (story prep)` (see step 3). Labelling those differently is what makes running unattended safe rather than reckless, because Jeroen can find every one of them at PR review and see that it was a tie rather than a finding.
...
**Never record a date on any entry.** No `DECIDED (research, 2026-…)`, no "checked <date>" on a source, no `created:` frontmatter. Git already tracks when every line was written; a second, hand-typed date only diverges from it. Record *that* the decision was made and by whom (research, story prep, or Jeroen), never *when*.

For a decision you made from research:

```
DECIDED (research): Validate `xsd_totalDigits` against the lexical value, not the parsed number.
...
Sources: w3.org/TR/xmlschema11-2/#rf-totalDigits, vee-validate docs on string-first field values
```

For a post-approval tie you broke on ladder rung 4 or 5, where there was no winner on the merits:

```
ASSUMED (story prep): Expose the new setting on `DynamicFormSettings` rather than as a `DynamicForm` prop.
...
```
DECIDED (Jeroen): <the decision, in one or two sentences>
```
...
Same rules apply: one line each, delete the section when it empties, and promote an entry to `DECIDED (Jeroen)` if he rules on it later.
